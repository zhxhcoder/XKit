 ...
 * 简单的网络封装用handler传回okttp网络请求
 */

public class OkHttpCall {
    private static volatile OkHttpCall mInstance;
    private OkHttpClient mOkHttpClient;
    private Handler mHandler;

    public static OkHttpCall getInstance(Context context) {
        if (mInstance == null) {
            synchronized (OkHttpCall.class) {
                if (mInstance == null) {
                    mInstance = new OkHttpCall(context);
                }
            }
        }
        return mInstance;
    ...
    private OkHttpCall(Context context) {
